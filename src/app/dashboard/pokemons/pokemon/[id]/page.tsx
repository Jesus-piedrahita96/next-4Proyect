...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const {id, name} = await getDetailPokemon(params.id)
    return {
      title: `#${id} - ${name}`,
...
const getDetailPokemon = async (id: string): Promise<DetailPokemon> => {
  //todo: cambiar el cache a futuro
  const detailPokemon = await (await fetch(`https://pokeapi.co/api/v2/pokemon/${id}/`, { cache: 'force-cache' })).json()
